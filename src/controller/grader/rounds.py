...
rounds.py
~~~~~~~~~~~~~~~~~
Implements the APIs for Grader control of adding discussion rounds.
...
class Rounds(webapp2.RequestHandler):
    def get(self):
        ...
        template = utils.jinja_env().get_template('grader/rounds.html')
    ...
    def post(self):
        ...
        if not action:
            ...
            if action == 'add':
                ...
                self.add_leadin_summary(grader)
            elif action == 'end-current-round':
                self.end_current_round(grader)
                ...
                self.delete_round(grader, round_id)
            elif action == 'change':
                # Send the id of the round to be edited
                round_id = int(self.request.get('round_id'))
                self.edit_round(grader, round_id)
            elif action == 'start':
                ...
                self.toggle_round_structure(grader)
            else:
