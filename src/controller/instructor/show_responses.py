...
class ShowResponses(webapp2.RequestHandler):
    def get(self):
        ...
        # Grab all the courses and assignments for the logged in instructor
        template_values = utils.get_template_all_courses_and_assignments(instructor,
                                                                      course_name.upper(),
...
class DataExport(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        writer.writerow([instructor.export_course, course.export_assignment, ])
        ...
        # writer.writerow([instructor.export_course, course.export_assignment, instructor.email, ])
...
class HtmlExport(webapp2.RequestHandler):
    def get(self, type):
        ...
        groups = model.Group.query(ancestor=assignment.key).fetch()
        template_values = {}
        output_students = []
        output_seq_rounds = {}
        output_options = {}
        ...
        #             ancestor=model.Round.get_by_id(1, parent=current_assignment.key).key).fetch() #fetches round object
        #
        #         no_answer_students = []
        ...
        for i in export_rounds.keys():
            ...
            #### added for reference
            #student_info[i] = utils.get_student_info(students[i].email, assignment.students)
            ...
            for j in export_rounds[i]:
                ...
                if posts:
                    ...
                    responses = model.Response.query(
                        ancestor=model.Round.get_by_id(1, parent=assignment.key).key).fetch()
                    no_answer_students = []
                    for stu in assignment.students:
                        flag = True
                        # Loop over the responses
        ...
        for g in range(1, assignment.groups + 1):
            # And loop over the rounds (indexed by 1)
            for r in range(1, assignment.rounds + 1):
        ...
        for r in range(1, assignment.rounds + 1):
            ...
            responses = model.Response.query(
                ancestor=model.Round.get_by_id(r, parent=assignment.key).key).fetch()
        ...
        for group in groups:
            answer, did_not_participate = group_comments(group, assignment, initial)
            initial_answers.append(answer)
            final_answers = group_comments(group, assignment, model.Round.get_by_id(3, parent=assignment.key))
